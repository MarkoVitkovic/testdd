...
const GradeCodes = () => {
    ...
    useEffect(() => {
        ...
        const channeldelete = pusher.subscribe('gradecode-deleted')
        const channelcreate = pusher.subscribe('gradecode-created')
        const channelupdate = pusher.subscribe('gradecode-updated')

        channeldelete.bind('gradecode-deleted-event', data => {
            setCodes((prev) => {
        ...
        channelcreate.bind('gradecode-created-event', data => {
            getItem(data.id, 'created')
        })

        channelupdate.bind('gradecode-updated-event', data => {
    ...
    const createGradeCode = async (e) => {
        ...
        await axios.post(`/api/grade-codes`, formData, config).then(({ data }) => {
            Swal.fire({
                icon: "success",
    ...
    const columns = [
        ...
        },
        {
            ...
            flex: 0.5,
            renderCell: (params) => <CodesActions params={params} config={config} setIsLoading={setIsLoading}/>
        }
    ]
                ...
                <div className='p-5'>
                    <div className='pb-5 shadow-md mb-2 rounded-md'>
                        <div className='flex justify-start items-center gap-4 '>
                            <p className='font-bold roboto pl-5 color-fake'>{t('grade_codes')}</p>
                            <Tooltip title={t('create_grade_code')} placement='right'>
                                <div>
                        ...
                        <div className='flex justify-between items-end w-1/2'>
                            <div className='px-5 pt-5 w-full'>
                                <div className='flex justify-between items-center search'>
                                    <input type="text" placeholder={t('searchGradeCode')} className='w-full border-0 focus:ring-0 px-0' style={{ paddingBottom: '4px' }} onChange={(e) => setSearchCode(e.target.value)} />
                ...
                <Box sx={style}>
                    <div className='flex justify-between items-center p-5 pr-0 pt-0 pb-0' style={{ backgroundColor: '#336195', borderRadius: '5px 5px 0 0' }}>
                        <div className='flex gap-4 items-baseline'>
                            ...
                            <p className='text-xl roboto font-semibold text-white'>{t('create_grade_code')}</p>
                        </div>
...
export default GradeCodes

const CodesActions = (params) => {

    const { t } = useTranslation()

    const deleteItems = async (id) => {
        const isConfirm = await Swal.fire({
            title: t('title_delete') + t('del_gc') + params.params.row.code + "?",
            text: t('text_delete'),
            icon: 'warning',
        ...
        await axios.delete(`/api/grade-codes/${id}`, params.config).then(({ data }) => {
            Swal.fire({
                icon: "success",
                text: data.success.message
            })
