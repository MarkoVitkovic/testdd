...
const UpdateUnitOfMeasure = () => {
    ...
    const updateItem = async (e) => {
        ...
        await axios.put(`/api/units-of-measure/${params.id}`, formData, config).then(({ data }) => {
            Swal.fire({
                icon: "success",
                ...
                <div className='flex justify-between items-center p-5 pr-0 pt-0 pb-0' style={{ backgroundColor: '#336195', borderRadius: '5px 5px 0 0' }}>
                    <div className='flex gap-4 items-baseline'>
                        <div style={{ transform: "rotate(45deg)" }} className="font-semibold text-white">
                            <Link to="/units-of-measure"><i className="fa-solid fa-plus"></i></Link>
                        </div>
                        <p className='text-xl roboto font-semibold text-white'>{t('update_unit_of_measure')} - {code}</p>
                    </div>
