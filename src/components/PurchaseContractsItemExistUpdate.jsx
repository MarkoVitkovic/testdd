...
const PurchaseContractsItemExistUpdate = ({ setIsLoading, id, itemsSend, setItemsSend }) => {

    const { t } = useTranslation()
    const { config } = useStateContext()

    const [purchaseItems, setPurchaseItems] = useState(itemsSend)
    ...
    useEffect(() => {
        getItems(purchaseItems.map(({item_no}) => item_no))
        getPurchaseitems(id)
    }, [])
    ...
    const getPurchaseitems = async (id) => {
        await axios.get(`/api/purchase-contract-items?purchase_contract_id=${id}`, config)
            .then(res => {
                const shipTo = res.data
                setPurchaseItems(shipTo)
                setItemsSend(shipTo)
    ...
    const getItems = async (without = []) => {
        await axios.get(`/api/items?blocked_for_purchases=0&blocked=0&company_id=${localStorage.getItem('company_id')}`, config)
            .then(res => {
                const shipTo = res.data
    ...
    const updateItem = (id, quantity, unit_price) => {
        const purchaseItemsCopy = [...purchaseItems]
        const item = purchaseItemsCopy.find(item => item.id === id)

        item.quantity = quantity
        item.unit_price = unit_price
        item.total_cost = quantity * unit_price

        setPurchaseItems(purchaseItemsCopy)
        setItemsSend(purchaseItemsCopy)
    }
    ...
    const createItem = async (e) => {
        ...
        setPurchaseItems((prev) => [...prev, new_item])
    ...
    const shipToColumns = [
        ...
        },
        {
            ...
            flex: 0.5,
            renderCell: (params) => <DeleteItem params={params} setPurchaseItems={setPurchaseItems} purchaseItems={purchaseItems} setItemsSend={setItemsSend} itemsSend={itemsSend}  />
        }
    ]
            ...
            <div className='w-full p-5 pl-0'>
                <div className='pb-5 shadow-md mb-2 rounded-md'>
                    <div className='flex justify-start items-center gap-4 '>
                        <p className='font-bold roboto color-fake ml-5'>{t('purchaseContractItems')}</p>
                        <Tooltip title={t('addpurchaseContractItems')} placement='right'>
                            <div>
                ...
                <div>
                    <TableShared items={purchaseItems} columns={shipToColumns} />
                </div>
            </div>
                ...
                <Box sx={style}>
                    <div className='flex justify-between items-center p-5 pr-0 pt-0 pb-0' style={{ backgroundColor: '#336195', borderRadius: '5px 5px 0 0' }}>
                        <div className='flex gap-4 items-baseline'>
                            ...
                            <p className='text-xl roboto font-semibold text-white'>{t('addpurchaseContractItems')}</p>
                        </div>
                        <button type="button" disabled={disabled} onClick={createItem} style={{ width: '90px'}} className='text-white px-4 py-6 uppercase self-end roboto bg-zinc-900 hover:bg-zinc-700 disabled:bg-[hsla(0,0%,100%,.12)]'>
                            {t('add')}
                        </button>
                    </div>
                    <div>
...
export default PurchaseContractsItemExistUpdate
...
const DeleteItem = (params) => {
    ...
    const deleteItem = async (id) => {
        ...
        params.setPurchaseItems(params.purchaseItems.filter((row) => row.id !== id))
    }
