...
const AddSalesOrderItem = ({ open, handleOpen, purchaseOrderItems, setPurchaseOrderItems, disabled, setDisabled }) => {
    ...
    useEffect(() => {
        if(open) getItems(purchaseOrderItems.map(({item_no}) => item_no))
    }, [purchaseOrderItems])
    ...
    useEffect(() => {
        if(open) {
            getItems(purchaseOrderItems.map(({item_no}) => item_no))
        }
    }, [open])
            ...
            <Box sx={style}>
                <div className='flex justify-between items-center p-5 pr-0 pt-0 pb-0' style={{ backgroundColor: '#b7472a', borderRadius: '5px 5px 0 0' }}>
                    <div className='flex gap-4 items-baseline'>
                        ...
                        <p className='text-xl roboto font-semibold text-white'>{t('addpurchaseOrderItems')}</p>
                    </div>
