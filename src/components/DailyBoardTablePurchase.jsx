        ...
        <React.Fragment >
            <TableRow sx={{ '& > *': { borderBottom: 'unset !important' } }}>
                ...
                <TableCell sx={{ padding: '8px 16px' }}>
                    {
                        <TodaysSalesOrderAction id={row.id} status={row.po_status_id} />
                    }
                </TableCell>
                <TableCell sx={{ padding: '8px 16px' }}>
                            ...
                            <Table size="small" aria-label="purchases">
                                <TableHead>
                                    <TableRow>
                                        ...
                                        <TableCell sx={{ width: '20%' }}>{t('quantity_ordered')}</TableCell>
            ...
            <Paper sx={{ width: '100%', mb: 2 }}>
                <TableContainer>
                    <Table aria-label="collapsible table" stickyHeader>
                        <TableHead>
                            <TableRow>
                                <TableCell sx={{ width: '20%' }}>{t('purchase_order_no')}</TableCell>
                                <TableCell sx={{ width: '20%' }}>{t('vendor_name')}</TableCell>
                                <TableCell sx={{ width: '30%' }}>{t('requested_collection_date')}</TableCell>
            ...
            <div className='flex gap-1'>
                <Tooltip title="BOL/POD" placement='top'>
                    <div style={{ color: 'rgba(0,0,0,.54)' }}>
                        <a href={process.env.REACT_APP_BACKEND_URL + `/api/purchase-orders/${id}/proof-of-delivery`} target="_blank"><span style={{ cursor: 'pointer' }} className="flex justify-center items-center hover:rounded-full icons p-2 hover:bg-zinc-200"><i className="fa-solid fa-print"></i></span></a>
                    </div>
                </Tooltip>
                {status === 1 || status === 3 ?
